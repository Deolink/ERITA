...
def _u32(data: bytes, offset: int, label: str) -> int:
    if offset < 0 or offset + 4 > len(data):
        raise BnkMergeError(f"{label} troncato all'offset {offset}.")
...
def parse_bnk(data: bytes) -> tuple[BnkSection, ...]:
    ...
    while offset < len(data):
        if offset + 8 > len(data):
            raise BnkMergeError(
                f"Intestazione di sezione BNK troncata all'offset {offset}."
            )
        chunk_id = data[offset : offset + 4]
        if any(value < 0x20 or value > 0x7E for value in chunk_id):
            raise BnkMergeError(
                f"Identificatore di sezione BNK non valido all'offset {offset}: "
                f"{chunk_id!r}."
            )
        size = _u32(data, offset + 4, f"Dimensione della sezione {chunk_id!r}")
        data_start = offset + 8
        data_end = data_start + size
        if data_end > len(data):
            raise BnkMergeError(
                f"La sezione {chunk_id!r} supera la fine del BNK "
                f"({data_end} > {len(data)})."
            )
        ...
        if chunk_id in _SINGLETON_CHUNKS and counts[chunk_id] > 1:
            raise BnkMergeError(f"Sezione BNK duplicata: {chunk_id!r}.")
        offset = data_end

    if not sections or sections[0].chunk_id != b"BKHD":
        raise BnkMergeError("BKHD deve essere la prima sezione del BNK.")
    if counts.get(b"BKHD") != 1:
        raise BnkMergeError("Il BNK deve contenere esattamente una sezione BKHD.")
    if counts.get(b"HIRC") != 1:
        raise BnkMergeError("Il BNK deve contenere esattamente una sezione HIRC.")
    if (counts.get(b"DIDX", 0), counts.get(b"DATA", 0)) not in {
        (0, 0),
        (1, 1),
    }:
        raise BnkMergeError("DIDX e DATA devono esistere insieme e senza duplicati.")
...
def parse_hirc(data: bytes) -> tuple[HircObject, ...]:
    ...
    for index in range(count):
        if offset + 5 > len(data):
            raise BnkMergeError(f"Oggetto HIRC[{index}] troncato.")
        type_id = data[offset]
        object_size = _u32(data, offset + 1, f"Dimensione HIRC[{index}]")
        if object_size < 4:
            raise BnkMergeError(
                f"L'oggetto HIRC[{index}] non possiede un id a 32 bit."
            )
        end = offset + 5 + object_size
        if end > len(data):
            raise BnkMergeError(
                f"L'oggetto HIRC[{index}] supera la fine della sezione."
            )
        object_id = _u32(data, offset + 5, f"Id HIRC[{index}]")
        if type_id == 2:
            if object_id in type_two_ids:
                raise BnkMergeError(
                    f"Oggetto Sound HIRC Type=2 duplicato: {object_id}."
                )
    ...
    if offset != len(data):
        raise BnkMergeError(
            f"L'HIRC ha {len(data) - offset} byte fuori dall'elenco degli oggetti."
        )
    return tuple(result)
...
def _validate_bank_identity(
    ...
    if len(vanilla_header) < 8 or len(payload_header) < 8:
        raise BnkMergeError("BKHD troncato: versione e bank id sono obbligatori.")
    ...
    if vanilla_identity != payload_identity:
        raise BnkMergeError(
            "Il payload appartiene a un altro banco o versione Wwise "
...
def _parse_media(didx: bytes, data: bytes, label: str) -> tuple[_Media, ...]:
    if len(didx) % 12:
        raise BnkMergeError(
            f"Il DIDX {label} ha una dimensione non valida ({len(didx)} byte)."
        )
    ...
    for index, offset in enumerate(range(0, len(didx), 12)):
        ...
        if media_id in media_ids:
            raise BnkMergeError(f"Media DIDX {label} duplicata: {media_id}.")
        media_ids.add(media_id)
        end = data_offset + size
        if data_offset < previous_end:
            raise BnkMergeError(
                f"DIDX {label}[{index}] si sovrappone o esce dall'ordine precedente."
            )
        if end > len(data):
            raise BnkMergeError(
                f"DIDX {label}[{index}] supera DATA ({end} > {len(data)})."
            )
        if any(data[previous_end:data_offset]):
            raise BnkMergeError(
                f"Il padding DATA {label} prima della media {media_id} non è zero."
            )
        result.append(_Media(media_id, data_offset, data[data_offset:end]))
        previous_end = end

    if any(data[previous_end:]):
        raise BnkMergeError(f"Il padding finale DATA {label} non è zero.")
    return tuple(result)
...
def _infer_alignment(media: tuple[_Media, ...]) -> int:
    ...
    for alignment in (4096, 2048, 1024, 512, 256, 128, 64, 32, 8, 4, 2, 1):
        if _uses_alignment(media, alignment):
            return alignment
    raise BnkMergeError("Non è stato possibile dedurre l'allineamento di DATA vanilla.")
...
def _merge_media(
    ...
    payload_media = _parse_media(payload_didx, payload_data, "del payload")
...
def _serialize_sections(sections: tuple[BnkSection, ...]) -> bytes:
    ...
    for section in sections:
        if len(section.data) > _U32_MAX:
            raise BnkMergeError(f"La sezione {section.chunk_id!r} supera i 4 GiB.")
