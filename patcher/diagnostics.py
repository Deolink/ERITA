"""Diagnostica locale e opzionale per l'installer ERITA.

Il report usa un elenco chiuso di campi. Non invia mai dati, non consulta
variabili d'ambiente e non include il percorso scelto dall'utente. Il
chiamante decide se il testo verrà copiato, salvato o condiviso.
"""
...
REPORT_SCHEMA = "erita-diagnostic"
...
def sanitize_text(value: object, *, max_chars: int = MAX_GENERIC_FIELD_CHARS) -> str:
    """Rimuove dati personali, percorsi e segreti da un campo libero.

    La funzione privilegia la privacy rispetto alla conservazione di ogni parola:
    trovando un percorso assoluto senza virgolette, può oscurare anche il resto
    della frase.
    """
    ...
    if _TRACEBACK_MARKER.casefold() in text.casefold():
        ...
        text = text[:marker_at] + "[TRACEBACK OMESSO]"
    text = _CONTROL_CHARACTER_RE.sub(" ", text)
    text = _URL_RE.sub("[URL]", text)
    text = _BEARER_RE.sub("Bearer [SEGRETO]", text)
    text = _SECRET_ASSIGNMENT_RE.sub(
        lambda match: _replacement_with_label(match, "SEGRETO"), text
    )
    text = _IDENTITY_ASSIGNMENT_RE.sub(
        lambda match: _replacement_with_label(match, "IDENTITÀ"), text
    )
    text = _KNOWN_TOKEN_RE.sub("[SEGRETO]", text)
    text = _JWT_RE.sub("[SEGRETO]", text)
    text = _EMAIL_RE.sub("[EMAIL]", text)
    text = _STEAM_ID_ASSIGNMENT_RE.sub("SteamID=[STEAM_ID]", text)
    text = _STEAM_ID_RE.sub("[STEAM_ID]", text)
    text = _AT_MENTION_RE.sub("[MENZIONE]", text)
    text = _ENV_PATH_RE.sub("[PERCORSO]", text)
    text = _QUOTED_ABSOLUTE_PATH_RE.sub("[PERCORSO]", text)
    text = _WINDOWS_PATH_RE.sub("[PERCORSO]", text)
    text = _WINDOWS_ROOT_PATH_RE.sub("[PERCORSO]", text)
    text = _NETWORK_PATH_RE.sub("[PERCORSO]", text)
    text = _HOME_PATH_RE.sub("[PERCORSO]", text)
    text = _POSIX_PATH_RE.sub("[PERCORSO]", text)
...
def _encode_bounded_report(report: dict[str, object]) -> str:
    ...
    report["recent_log"] = ["[LOG OMESSO: limite del report]"]
    result = encode()
    if len(result.encode("utf-8")) <= MAX_REPORT_BYTES:
        return result
    ...
    if isinstance(error, dict):
        error["message"] = "[MESSAGGIO OMESSO: limite del report]"
    ...
    result = encode()
    if len(result.encode("utf-8")) > MAX_REPORT_BYTES:
        raise ValueError("Il report sanitizzato ha superato il limite sicuro.")
    return result
